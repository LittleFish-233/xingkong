...
{
    public static class Sys_infor
    {
        /// <summary>
        /// 获取Java信息 0java路径 1java版本
        /// </summary>
        /// <returns></returns>
        public static string[] java_path()
        {
            ...
            try
            {
                string[] JavaPaths = registryKey.OpenSubKey(@"SOFTWARE\JavaSoft\Java Runtime Environment").GetSubKeyNames();
                string[] JavaVersions = registryKey.OpenSubKey(@"SOFTWARE\JavaSoft\").GetSubKeyNames();
                string JavaPath = registryKey.OpenSubKey(@"SOFTWARE\JavaSoft\Java Runtime Environment\" + JavaPaths[0]).GetValue("JavaHome") + "\\bin\\javaw.exe";
                string JavaVersion = "";
                for (int i = 0; i < JavaVersions.Length; i++)
                {
                    if (JavaVersions[i] == "Java Runtime Environment")
                    {
                        JavaVersion = registryKey.OpenSubKey(@"SOFTWARE\JavaSoft\" + JavaVersions[i]).GetValue("CurrentVersion").ToString();
                    }
                }
                result[0] = JavaPath;
                result[1] = JavaVersion;
                return result;
            }
            catch (Exception exc)
            {
